...
p1 <- ggplot(subset(smooth_acceleration, Subject==1 & Item.number ==10), aes(x=as.numeric(Time.Step), y=as.numeric(Acceleration_Smooth), color=Polarity, group=grp)) +
  geom_line(alpha=.5) +
  geom_point()+
  ggtitle('')+
  theme_minimal() +
...
p2 <- ggplot(subset(normalized_positions.plot, Subject==1 & Item.number ==10), aes(x=as.numeric(Time.Step), y=as.numeric(Acceleration), color=Polarity, group=grp)) +
  geom_line(alpha=.5) +
  geom_point()+
  ggtitle('')+
  theme_minimal() +
...
acceleration_mean.subject <-   ddply(subset(smooth_acceleration, Polarity!='uncertain'), c("Polarity", "Time.Step.Onset", "Subject"),
                                     function(smooth_acceleration)c(response=mean(smooth_acceleration$Acceleration_Smooth, na.rm=T)))
...
acceleration_mean.subject <-   ddply(subset(smooth_acceleration, Polarity!='uncertain'), c("Polarity", "Time.Step.Onset", "Subject", "PointChange.Time_cut"),
                                     function(smooth_acceleration)c(response=mean(smooth_acceleration$Acceleration_Smooth, na.rm=T)))
