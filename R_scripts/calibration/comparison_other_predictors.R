...
for (b in 1: length(bins)) {
  ...
  calibrationTest <- subset(calibration_data, id %in% bins[[b]]$id)
    ...
    ## LDA with only Velocity
      ...
      #assign(paste0('lda_vel.roc.te',b), lda.roc.te)
   ...
   ## LDA with DeltaDelta
    ...
    ## LDA with Delta
    ###  TRAINING + TEST
    LDA_training.delta(calibrationTrain)
    LDA_test.delta(calibrationTest, v_lda, b_lda, m_pca, all_data_columns, n_pca)
    
    #ROC and AUC
    lda.score.te <- lda_measure_te.df$lda_measure
    lda.label.te <- lda_measure_te.df$Deviation
    lda.roc.te <- roc(lda.label.te, lda.score.te)
    auc.bins$lda.delta[b] <- lda.roc.te$auc
    #assign(paste0('lda_delta.roc.te',b), lda.roc.te)
