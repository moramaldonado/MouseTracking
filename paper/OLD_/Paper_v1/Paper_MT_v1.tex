...
\label{sec:LDA}
Different `quasi-decisions' (i.e. \textsc{Decision type}s) have a different impact on mouse trajectories, as observed in \Cref{fig:mean.trajectories.calibration}. To identify the features characteristic of each class (\textit{switched} vs. \textit{straightforward}), we use a Linear Discriminant Analysis method for classification. 
...
A scheme of the procedure is provided in \Cref{fig:diagram}
...
%Nodes
\node[squarednode]      (pca)                              {\begin{tabular}{c} \textbf{13 Principal Components} \\ (PCA: Maximase variance)\end{tabular}};
...
\node[squarednode2]        (ldameasure)       [below=of lda] {\begin{tabular}{c} \textbf{LDA Measure}\\ Projection of the PCs on the \\ axis that maximise the separability  \end{tabular}};
...
The mean AUCs values for the LDA, the baseline and the topline in each bin are given in \Cref{DIST:AUC}a. 
...
The performance of these additional classifiers was diagnosed in the same way as before, by computing the AUC for each of the 10 test bins. \Cref{DIST:AUC}b illustrates the mean AUCs values for each of these classifiers, together with the original LDA, the baseline and topline. 
...
(\Cref{fig:traditional-measures}; REF). For both measures, higher values are associated with higher deviation peaks towards the alternative; values closer to zero (or below) suggest trajectory close to ideal. 
Another frequently used measure to estimate the complexity of the trajectory is based on quantifying the number of times the trajectories goes back and forth along the x-axis (horizontal flips, Dale and Duran 2011), as illustrated in \Cref{fig:traditional-measures}.
...
All participants responded correctly more than 75\% of the time. No participant was discarded based on accuracy.  Only accurate trials were taken into account for the analysis. \Cref{fig:mean.trajectory-negation} illustrates mean trajectories for the four possible trial conditions.
\begin{figure}
\centering
...
In order to estimate the power of the classification, we measured the performance after reducing the sample size. \Cref{fig:permutation_AUC_negation}A shows the mean AUC values obtained after applying the same procedure to different sample sizes. 
...
Lastly, while the classifiers' comparison in  \Cref{DIST:AUC} indicated that relative spatio-temporal features, such as distance-based acceleration and speed, were not essential for the classification of `quasi-deicisions', these features do seem to play a role in the classification of sentence-verification data. When comparing the performance of both LDAs in \Cref{fig:permutation_AUC_negation}, we observe that the \emph{full} classifier --which takes all features as predictors-- makes a better distinction than the simplified one.   
