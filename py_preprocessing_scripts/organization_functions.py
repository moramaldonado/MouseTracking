...
def exporting_data(path,all_trials, info):
    ...
    with open(info_name, 'w') as m:
        ...
        for j in range(len(info)):
            writer.writerow((info[j]['subject'], info[j]['file'], info[j]['gender'],info[j]['age'],info[j]['total_time'],info[j]['points'], info[j]['clicker'][0],
                                info[j]['handedness'], info[j]['language'], info[j]['mobile'], info[j]['normalized_button_size'], info[j]['portrait'], info[j]['touch'],info[j]['userAgent'], info[j]['windowWidth']))
    ...
    with open(name, 'w') as f:
        ...
        writer.writerow( ('Subject', 'Sentence_Type', 'Adjective', 'Polarity',
                          ...
                          'MaxDeviationBorder','MaxDeviationBorder.Time','MaxDeviationBorder.Time.Norm', 'Median.LogRatio',
                          'MaxRatio','MaxLogRatio','MaxRatio.Time','MaxRatio.Time.Norm','MaxDifference','MaxDifference.Time','MaxDifference.Time.Norm',
                          'AccPeak','AccPeak.Time','AccPeak.Time.Norm', 'Int.Ratio.AccPeak', 'Int.LogRatio.AccPeak','Int.LogRatio.Fin','Int.Difference.AccPeak','Int.X.AccPeak','AUC','Local.Maxima.Acc','Len.Local.Maxima','Delay', 'Ratio.Acc') )
            ...
            for t in range(len(all_trials[i])):
                    ...
                    writer.writerow((str(i), all_trials[i][t]['type'], all_trials[i][t]['adjective'], all_trials[i][t]['polarity'],
                                      all_trials[i][t]['expected_response'],
                                        all_trials[i][t]['value'], all_trials[i][t]['accuracy'], all_trials[i][t]['RT'], all_trials[i][t]['maxDeviation'][0], all_trials[i][t]['maxDeviation'][1],all_trials[i][t]['maxDeviation'][2], all_trials[i][t]['maxDeviationBorder'][0], all_trials[i][t]['maxDeviationBorder'][1],all_trials[i][t]['maxDeviationBorder'][2], all_trials[i][t]['median_ratio_log'], all_trials[i][t]['max_ratio'][0],all_trials[i][t]['max_ratio_log'][0],all_trials[i][t]['max_ratio'][1],all_trials[i][t]['max_ratio'][2],all_trials[i][t]['max_difference'][0],all_trials[i][t]['max_difference'][1],all_trials[i][t]['max_difference'][2], all_trials[i][t]['max_smooth_acceleration'][0],all_trials[i][t]['max_smooth_acceleration'][1],all_trials[i][t]['max_smooth_acceleration'][2], all_trials[i][t]['integral_ratio_on_max_smooth_acceleration'],all_trials[i][t]['integral_ratio_log_on_max_smooth_acceleration'],all_trials[i][t]['integral_ratio_log_on_fin'],all_trials[i][t]['integral_difference_on_max_smooth_acceleration'],all_trials[i][t]['integral_X_on_max_smooth_acceleration'],all_trials[i][t]['integral_X_on_fin'],all_trials[i][t]['local_maxima'],len(all_trials[i][t]['local_maxima']),all_trials[i][t]['delay'],all_trials[i][t]['ratio_log_in_max_smooth_acceleration']))
